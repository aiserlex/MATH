...
class Main(Window):
    ...
    def _draw(self):
        ...
        if "Up" in self._history:
            self._angle_z += 2

        if "Down" in self._history:
            self._angle_z -= 2
        ...
        Transformations.rotate_x(cube.points, self._angle_x)
        Transformations.rotate_y(cube.points, self._angle_y)
        Transformations.rotate_z(cube.points, self._angle_z)
        ...
        for triangle in cube.triangles:
            ptriangle = self._projector.project_triangle(triangle)

            Transformations.resize_object(ptriangle.points, scale=self._dist)
            Transformations.move_object(ptriangle.points, x=200, y=200)

            ptriangles.append(ptriangle)

        for priangle in ptriangles:
